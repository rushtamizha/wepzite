...
import { MessageSquareCode, CheckCircle2, MessageSquare, ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function WhatsappAutomationPage() {
  const techStack = ["Lead Categorization", "Frictionless Booking", "API Routing"];
  const bulletPoints = [
    "Instant Automated Lead Classification Mechanics",
    "Frictionless Instant Booking Validation Routes",
    "Robust Cloud Webhook Trigger API Architectures",
    "Real-Time retention-optimized customer response tunnels"
  ];

  const triggerWhatsApp = () => {
    const msg = encodeURIComponent("Hi Wepzite! I want to inquire about a strategy for WhatsApp Business Automation funnels.");
      ...
      <div className="w-full max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6">
            <span className="text-[10px] font-black tracking-widest text-sky-600 bg-sky-50 px-3 py-1 rounded-full uppercase border border-sky-100">Instant Connect</span>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 uppercase tracking-tight leading-none">Smart WhatsApp Automation</h1>
          ...
          <div className="lg:col-span-5">
            <div className="w-full aspect-square rounded-3xl overflow-hidden border border-sky-100 bg-slate-50 p-4 shadow-sm">
              <img src="image_agent_tag_17038675738162256343" alt="WhatsApp Business Automated Phone Funnel Dashboard" className="w-full h-full object-cover rounded-2xl" />
            </div>
          </div>
        </div>
      </div>
