...
import { ShoppingBag, CheckCircle2, MessageSquare, ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function EcommercePage() {
  const techStack = ["Secure Cart Logic", "Custom CMS", "Payment APIs", "Analytics"];
  const bulletPoints = [
    "Frictionless Multi-Stage Single-Page Checkout Systems",
    "Highly Scalable Secure Product Categorization Logic",
    "Seamless Direct Razorpay / Payment Gateway Pipelines",
    "Live Real-Time Inventory Control Metrics Dashboards"
  ];

  const triggerWhatsApp = () => {
    const msg = encodeURIComponent("Hi Wepzite! I want to inquire about a strategy for high-converting E-Commerce platforms.");
      ...
      <div className="w-full max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6">
            <span className="text-[10px] font-black tracking-widest text-sky-600 bg-sky-50 px-3 py-1 rounded-full uppercase border border-sky-100">Revenue Engine</span>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 uppercase tracking-tight leading-none">High-Converting E-Commerce</h1>
            <p className="text-slate-500 text-sm sm:text-base font-medium normal-case leading-relaxed">
              Frictionless checkout paths, highly reliable modern cart logic, and robust global payment gateway system integrations. Built to handle heavy user traffic without breaking a sweat.
            </p>
          ...
          <div className="lg:col-span-5">
            <div className="w-full aspect-square rounded-3xl overflow-hidden border border-sky-100 bg-slate-50 p-4 shadow-sm">
              <img src="image_agent_tag_17038675738162255218" alt="Modern Ecommerce Interface Vector Layout" className="w-full h-full object-cover rounded-2xl" />
            </div>
          </div>
        </div>
      </div>
