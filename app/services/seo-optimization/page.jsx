...
import { SearchCode, CheckCircle2, MessageSquare, ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function SeoOptimizationPage() {
  const techStack = ["Schema Markup", "Search Console Mapping", "Core Web Vitals"];
  const bulletPoints = [
    "Advanced Structured JSON-LD Data Schema Markups",
    "Automated Google Search Console Index Mapping Filters",
    "Precision Performance Engineering for Core Web Vitals",
    "Organic Semantic Keyword Visibility Optimization Architecture"
  ];

  const triggerWhatsApp = () => {
    const msg = encodeURIComponent("Hi Wepzite! I want to inquire about a strategy for High-Level SEO Optimization.");
      ...
      <div className="w-full max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6">
            <span className="text-[10px] font-black tracking-widest text-sky-600 bg-sky-50 px-3 py-1 rounded-full uppercase border border-sky-100">Rank Max</span>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 uppercase tracking-tight leading-none">High-Level SEO Optimization</h1>
            <p className="text-slate-500 text-sm sm:text-base font-medium normal-case leading-relaxed">
              Advanced data structuring, complete meta-tag configurations, and automated Google Search Console index maps. On-page SEO strategies that get your business discovered organically.
            </p>
          ...
          <div className="lg:col-span-5">
            <div className="w-full aspect-square rounded-3xl overflow-hidden border border-sky-100 bg-slate-50 p-4 shadow-sm">
              <img src="image_agent_tag_17038675738162255968" alt="Google Search Console Audit Metrics Layout" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </div>
